...
const STATUS_CONCILIACAO_BADGES = {
  PROCESSADO: 'bg-green-100 text-green-800 dark:bg-emerald-900/30 dark:text-emerald-300',
  COM_DIVERGENCIAS: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
...
const STATUS_ITEM_BADGES = {
  CONCILIADO: 'bg-green-100 text-green-800 dark:bg-emerald-900/30 dark:text-emerald-300',
  FALTANDO_BANCO: 'bg-yellow-100 text-yellow-800 dark:bg-amber-900/30 dark:text-amber-300',
  FALTANDO_SISTEMA: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
}
...
function Badge({ status, map, labels }) {
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${map[status] || 'bg-gray-100 text-gray-600 dark:bg-navy-700 dark:text-slate-400'}`}>
...
export default function Conciliacao() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Conciliação Bancária</h1>
        <p className="text-sm text-gray-500 mt-0.5 dark:text-slate-400">Envie extratos e concilie com o Livro Caixa</p>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-slate-200">Histórico de Conciliações</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400 dark:text-slate-500">
          <Inbox size={40} className="mx-auto mb-2 opacity-50 dark:text-navy-500" />
                ...
                <Card>
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900 truncate dark:text-slate-100">{item.conta_nome}</p>
                      <p className="text-xs text-gray-500 mt-0.5 dark:text-slate-400">{formatarPeriodo(item.periodo)}</p>
                    </div>
                    <Badge status={item.status} map={STATUS_CONCILIACAO_BADGES} labels={STATUS_CONCILIACAO_LABELS} />
                  </div>
                  <div className="mt-2 flex justify-between items-center text-sm">
                    <span className="text-gray-500 dark:text-slate-400">Banco: <span className="font-mono text-gray-700 dark:text-slate-300">{BRL(item.total_banco)}</span></span>
                    <span className="text-gray-500 dark:text-slate-400">Sistema: <span className="font-mono text-gray-700 dark:text-slate-300">{BRL(item.total_sistema)}</span></span>
                  </div>
                  {item.divergencias > 0 && (
                    <p className="text-xs text-red-600 font-medium mt-1 dark:text-red-400">{item.divergencias} divergência(s)</p>
                  )}
                </Card>
              </div>
            ))}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200 dark:bg-navy-900 dark:border-navy-600">
                      <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Período</th>
                      ...
                      <th className="text-right px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Total Banco</th>
                      <th className="text-right px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Total Sistema</th>
                      <th className="text-right px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Divergências</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Processado em</th>
                      <th className="px-4 py-3" />
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-navy-700">
                    {items.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50 cursor-pointer dark:hover:bg-navy-700/60" onClick={() => abrirDetalhe(item)}>
                        <td className="px-4 py-3 text-gray-600 dark:text-slate-400">{formatarPeriodo(item.periodo)}</td>
                        <td className="px-4 py-3 text-gray-600 dark:text-slate-400">{item.conta_nome}</td>
                        <td className="px-4 py-3">
                          <Badge status={item.status} map={STATUS_CONCILIACAO_BADGES} labels={STATUS_CONCILIACAO_LABELS} />
                        </td>
                        <td className="px-4 py-3 text-right font-mono text-gray-700 dark:text-slate-300">{BRL(item.total_banco)}</td>
                        <td className="px-4 py-3 text-right font-mono text-gray-700 dark:text-slate-300">{BRL(item.total_sistema)}</td>
                        <td className={`px-4 py-3 text-right ${item.divergencias > 0 ? 'font-semibold text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-slate-500'}`}>
                          {item.divergencias}
                        </td>
                        <td className="px-4 py-3 text-gray-500 text-xs dark:text-slate-400">{formatarDataHora(item.processado_em)}</td>
                        <td className="px-4 py-3 text-right"><ChevronRight size={16} className="text-gray-400 dark:text-slate-500" /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <Modal title="Nova Conciliação" onClose={closeModal} maxW="max-w-lg">
          <form onSubmit={handleUpload} className="space-y-4">
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Extrato Bancário (PDF)</label>
              <input
                type="file"
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                value={form.periodo}
                onChange={handleChange}
                className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors duration-150 dark:border-navy-500 dark:bg-navy-800 dark:text-slate-100 dark:focus:ring-violet-500"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-navy-500 dark:bg-navy-800 dark:focus:ring-violet-500"
              />
              <label htmlFor="auto" className="text-sm font-medium text-gray-700 dark:text-slate-300">
...
function DetalheConciliacao({ conciliacao, onClose, showToast, onAtualizar }) {
  ...
  const linhaClasse = (item) => {
    if (item.status === 'FALTANDO_BANCO') return 'bg-yellow-50 dark:bg-amber-950/20'
    if (item.status === 'FALTANDO_SISTEMA' && !item.confirmado) return 'bg-red-50 dark:bg-red-950/20'
    return 'hover:bg-gray-50 dark:hover:bg-navy-700/60'
  }
    ...
    <Modal title="Detalhe da Conciliação" onClose={onClose} maxW="max-w-4xl">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div>
          ...
          <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">{conciliacao.conta_nome}</p>
        </div>
        <div>
          <p className="text-xs text-gray-500 dark:text-slate-400">Período</p>
          <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">{formatarPeriodo(conciliacao.periodo)}</p>
        </div>
        <div>
        ...
        <div>
          <p className="text-xs text-gray-500 dark:text-slate-400">Divergências</p>
          <p className={`text-sm font-bold ${divergencias > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-emerald-400'}`}>{divergencias}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-4 pb-4 border-b border-gray-100 dark:border-navy-700">
        <div className="rounded-xl p-3 bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400 dark:text-slate-500">
          <Inbox size={32} className="mx-auto mb-2 opacity-40 dark:text-navy-500" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 dark:bg-navy-900 dark:border-navy-600">
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Status</th>
                <th className="text-right px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Ação</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-navy-700">
              {itens.map((item) => (
                <tr key={item.id} className={linhaClasse(item)}>
                  <td className="px-4 py-3 text-gray-600 whitespace-nowrap dark:text-slate-400">{formatarData(item.data_banco)}</td>
                  <td className="px-4 py-3 text-gray-900 max-w-[220px] truncate dark:text-slate-100">{item.descricao_banco}</td>
                  <td className="px-4 py-3">
                    <span className={`flex items-center gap-1 text-xs font-medium ${item.tipo === 'ENTRADA' ? 'text-green-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}>
                      {item.tipo === 'ENTRADA'
                  ...
                  <td className={`px-4 py-3 text-right font-semibold whitespace-nowrap ${item.tipo === 'ENTRADA' ? 'text-green-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}>
                    {item.tipo === 'ENTRADA' ? '+' : '-'}{BRL(item.valor)}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <Badge status={item.status} map={STATUS_ITEM_BADGES} labels={STATUS_ITEM_LABELS} />
                      {item.status === 'FALTANDO_BANCO' && <AlertTriangle size={14} className="text-yellow-600 dark:text-amber-400" />}
                    </div>
                  </td>
...
function PadroesTab({ showToast }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-slate-200">Padrões Seguros</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200 dark:bg-navy-900 dark:border-navy-600">
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Descrição</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Tipo</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-slate-400">Natureza</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-navy-700">
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-navy-700/60">
                      <td className="px-4 py-3 font-medium text-gray-900 max-w-[280px] truncate dark:text-slate-100">{item.descricao_padrao}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${item.tipo === 'ENTRADA' ? 'bg-green-100 text-green-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'}`}>
                          {item.tipo_label}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-gray-600 dark:text-slate-400">
                        {item.tipo === 'ENTRADA' ? item.natureza_label : <span className="text-gray-400 dark:text-slate-500">—</span>}
                      </td>
                      <td className="px-4 py-3 text-right">
        ...
        <Modal title={editingId ? 'Editar Padrão' : 'Novo Padrão'} onClose={closeModal}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className={form.tipo === 'SAIDA' ? 'opacity-50 cursor-not-allowed' : ''}>
              ...
              />
              {form.tipo === 'SAIDA' && (
                <p className="text-xs text-gray-400 mt-1 dark:text-slate-500">Aplicável apenas para Tipo = Entrada</p>
              )}
            </div>
            <div className="flex justify-end gap-3 pt-2">
              <Button type="button" variant="secondary" onClick={closeModal}>Cancelar</Button>
