    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          ...
          Fechar Caixa
          {sessao && <span className="text-gray-400 font-normal text-lg">— Sessão #{sessao.id}</span>}
        </h1>
        {sessao && (
      ...
      <Card>
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-700">Valor calculado em caixa (dinheiro físico)</span>
          <span className="text-lg font-bold text-gray-900 font-mono">{BRL(calculado)}</span>
        </div>
        ...
        <div className="space-y-4">
          ...
          {contagemNum !== null && (
            <div className={`flex items-center justify-between p-3 rounded-lg ${
              !temDiferenca ? 'bg-green-50' : diferencaGrande ? 'bg-red-50' : 'bg-yellow-50'
            }`}>
              <div className={`flex items-center gap-2 text-sm font-medium ${
                !temDiferenca ? 'text-green-700' : diferencaGrande ? 'text-red-700' : 'text-yellow-700'
              }`}>
                {!temDiferenca
                  ? <CheckCircle size={16} />
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none"
            />
          </div>
        </div>
      </Card>
